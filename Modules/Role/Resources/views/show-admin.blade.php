...
<div class="body flex-grow-1 px-3">
    <div class="container-lg">
        <div class="white-sec">
            <input type="button" value="Add New" onClick="window.location.href='{{ url('role/create-user') }}'" class="btn btn-primary" style="float:right"/>
            ...
            <table class="table table-hover" id="myTable">
                ...
                @foreach ($data as $v)
                    <tr>
                        ...
                        <td>{{$v['email']}}</td>
                        <td>{{$v['role']}}</td>
                        <td><a href="{{ url('role/user-details/')}}/{{$v['id']}}"><span class="fas fa-pen"></span></a> <a href="{{ url('role/user-delete/')}}/{{$v['id']}}" onclick="event.preventDefault(); confirmDelete('{{ $v['id'] }}');"><span class="fas fa-trash"></span></a> </td>
                    </tr>
                @endforeach
                </tbody>
            </table>
